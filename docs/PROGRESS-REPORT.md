...
| **Phase 2: 本地 → Overleaf 同步** | 反向同步（规划中） | 🚧 0% |
...
## 🚧 Phase 2: 开发计划（本地 → Overleaf）

### 目标

实现本地文件系统到 Overleaf 的反向同步，使 Claude Code 可以在本地编辑并自动同步到 Overleaf。
...
#### 2. 文件编辑推送
- [ ] 读取本地文件变化
- [ ] 通过 Overleaf API 推送更新
- [ ] 处理版本冲突

#### 3. 文件创建推送
- [ ] 检测新文件创建
- [ ] 调用 Overleaf API 创建文档
- [ ] 上传文件内容

#### 4. 文件删除推送
- [ ] 检测文件删除
- [ ] 调用 Overleaf API 删除文档
...
| 挑战 | 解决方案 |
|------|---------|
| **如何推送编辑** | 使用 Overleaf Private API `/project/{id}/doc/{doc_id}` |
| **如何获取 docId** | 维护 path → docId 反向映射 |
| **如何处理冲突** | 比较版本号，提示用户选择 |
| **如何保持连接** | 复用浏览器扩展的认证 |
